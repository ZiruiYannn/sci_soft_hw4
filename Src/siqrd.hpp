...
namespace kul {
    ...
    auto siqrd_J (ublas::vector<S> const& x, P alpha, P beta, P gamma, P delta, P mu) {
        ...
        auto SIR = x(0) + x(1) + x(3);

        ! frac{\partial S}{\partial * }
        J(0,0) = - beta * (x(1)/SIR - x(0) * x(1) /SIR ** 2);
        J(0,1) = - beta * (x(0)/SIR - x(0) * x(1) /SIR ** 2);
        J(0,2) = 0;
        J(0,3) = - beta * ( - x(0) * x(1) /SIR ** 2) + mu;
        J(0,4) = 0;
        ! frac{\partial I}{\partial *}
        J(1,0) = beta * (x(1)/SIR - x(0)*x(1)/SIR**2);
        J(1,1) = beta * (x(0)/SIR - x(0) * x(1)/ SIR ** 2) - gamma - delta - alpha;
        J(1,2) = 0;
        J(1,3) = beta * (- x(0)*x(1)/SIR**2 );
        J(1,4) = 0;
